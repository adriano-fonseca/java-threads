...
import future.process.stackoverflow.Question;
import future.process.util.AbstractFuturesTest;
/*
 * Much more in https://www.youtube.com/watch?v=-MBPQ7NIL_Y&t=1s
 * */
public class S04_FlatMap extends AbstractFuturesTest {
	...
	public void thenApplyIsWrong() throws Exception {
		final CompletableFuture<CompletableFuture<Question>> future = javaQuestions()
	...
	public void thenCompose() throws Exception {
		...
		final CompletableFuture<Question> questionFuture = java.thenCompose(doc -> findMostInterestingQuestion(doc));

		final CompletableFuture<String> answerFuture = questionFuture.thenCompose(question -> googleAnswer(question));

		final CompletableFuture<Integer> httpStatusFuture = answerFuture.thenCompose(answer -> postAnswer(answer));
	...
	public void chained() throws Exception {
		javaQuestions().thenCompose(doc -> findMostInterestingQuestion(doc))
				.thenCompose(question -> googleAnswer(question)).thenCompose(answer -> postAnswer(answer))
				.thenAccept(status -> {
					if (status == HttpStatus.OK.value()) {
						log.debug("OK");
					} else {
	...
	private CompletableFuture<Document> javaQuestions() {
		return CompletableFuture.supplyAsync(() -> client.mostRecentQuestionsAbout("java"), executorService);
	}
